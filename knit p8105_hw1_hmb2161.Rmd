...
* The size of the dataset is `nrow(probtwo_df)` rows and `ncol(probtwo_df)` columns
* The mean of the dataset is `mean(pull(probtwo_df, x)`, the median is `median(pull(probtwo_df, x)`, and the standard deviation is `sd(pull(probtwo_df, x)`
* The proportion of cases for which x + y >1 is `(length)(which(vec_logical)/((length)(vec_logical))`
